...
// METHODES PUBLIQUES
    ...
    get() {
        if(this._type===MoveType.PUT_PIECE){
            ...
            return "move from " + String.fromCharCode('a'.charCodeAt(0) + this._from.x) +(this._from.y) +
                "to" + String.fromCharCode('a'.charCodeAt(0) + this._to.x) + (this._to.y) + this._piece;
        }
    }
    ...
    isEqual(move2){
        return this.type===move2.type && this._from.x===move2._from.x && this._from.y===move2._from.y && this._to.x===move2._to.x &&
            this._to.y===move2._to.y && this._piece===move2._piece;
    }
