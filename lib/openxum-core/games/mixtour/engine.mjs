...
class Engine extends OpenXum.Engine {
  ...
  _get_possible_moves_in_direction(move_to_avoid, dest_tower, DirectionX, DirectionY) {
    ...
    for (let DirectionX = -1; DirectionX <= 1; ++DirectionX) {
      for (let DirectionY = -1; DirectionY <= 1; ++DirectionY) {
        if (DirectionX !== 0 || DirectionY !== 0) {
          move_list = move_list.concat(this._get_possible_moves_in_direction(move_to_avoid, dest_tower, DirectionX, DirectionY));
        }
      }
    }
    ...
    for (let i = 0; i < 5; ++i) {
      for (let j = 0; j < 5; ++j) {
        if (!this._search_tower(i, j)) {
          putting_list = putting_list.concat(new Move(MoveType.PUT_PIECE, {x: -1, y: -1}, {x: i, y: j}, 1));
        }
      }
    }
    return putting_list;
