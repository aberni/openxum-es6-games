...
class Engine extends OpenXum.Engine
{
    ...
    get_possible_move_list()
    {
        ...
        if(this._last_move) {
            const move_to_avoid = new Move(this._last_move.getType(), this._last_move.getTo(), this._last_move.getFrom(), this._last_move.getPiece());
        }
    ...
    move(move) {
        if(move.getType()===MoveType.PUT_PIECE){
            ...
            this._towers=this._towers.concat();
            ...
            let index_tower_to;
            for(let i=0; i<this._towers.length; ++i){
                if(this._towers.x=== move.getFrom().x && this._towers.y=== move.getFrom().y){
                    index_tower_from=i;
                }
                else{
                    if(this._towers.x=== move.getTo().x && this._towers.y=== move.getTo().y){
                        index_tower_to=i;
                    }
                }
            }
            ...
            if(move.getPiece()===0){
                ...
                this._towers[index_tower_to].pieces=this._towers[index_tower_to].pieces.slice(move.getPiece());
            }
    ...
    _next_color()
    {
        this._color === Color.WHITE ? Color.BLACK : Color.WHITE;
    }
    ...
    _remove_tower(index_tower_to_remove){
        this._towers=this._towers.splice(insex_tower_to_remove,1);
    }
