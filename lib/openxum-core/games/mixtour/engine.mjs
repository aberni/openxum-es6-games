...
class Engine extends OpenXum.Engine {
  ...
  winner_is() {
    if (this._black_points >= this._pts_to_win || this._black_points > this._white_points) {
      return Color.BLACK;
    }
    if (this._white_points >= this._pts_to_win || this._black_points > this._white_points) {
      return Color.WHITE;
    }
    return Color.NONE;
  }
  ...
  _add_point(pieces) {
    if (pieces[pieces.length - 1] === Color.WHITE) {
      this._white_points++;
      if (this._white_points >= this._pts_to_win) {
        this._phase = Phase.FINISH;
      }
    }
    else {
      this._black_points++;
      if (this._black_points >= this._pts_to_win) {
        this._phase = Phase.FINISH;
      }
    }
  }
  ...
  _get_possible_moves_in_direction(dest_tower, DirectionX, DirectionY) {
    ...
    if (distance === dest_tower.pieces.length - 1) {
