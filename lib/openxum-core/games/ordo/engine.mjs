...
class Engine extends OpenXum.Engine {
    ...
    _check_voisin(move){
        ...
        if(nbVdumove === 1)
        {
            for(let i = 0;i<=2;i++){
                for(let j = 0;j<=2;j++)
                {
                    if(movX-1+i >=0 && movY-1+j) {
                        if (this._is_connected(move.piece(), movX - 1 + i, movY - 1 + j) < 2) {
                            if (this._color === Color.BLACK) {
                                this._disconnect_black = true;
                            }
                            else {
    ...
    _get_possible_move_list(piece){
        ...
        if(piece._color ===this._color) {
            let pc = piece.coordinates();
            //Up
            if (pc.y() > 0 && this._disconnect_black === true && this._color === Color.BLACK) {
                for (let dy = 1; dy < pc.y()+1; dy++) {
            ...
            //UL
            if(pc.x() > 0 && pc.y() > 0  && this._disconnect_black === true && this._color === Color.BLACK )
            {
                for(let dx = 1,dy = 1; dx < pc.x()+1 && dy < pc.y()+1;dx++,dy++){
            ...
            //RU
            if(pc.x() <= 9 && pc.y() > 0  && this._disconnect_black === true && this._color === Color.BLACK)
            {
                for(let dx = 1,dy = 1; dx < 10-pc.x() && dy < pc.y()+1;dx++,dy++){
                    //let moves_sample = moves;
                    moves = this._possible_moves_function(piece, pc.x()+dx, pc.y()-dy, moves);
