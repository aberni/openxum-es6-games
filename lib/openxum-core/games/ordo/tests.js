...
QUnit.test( " _verify_moving OK", function( assert ) {
  let e = new window.OpenXum.Ordo.Engine();
  e._color = 2;
...
/*
QUnit.test( " _check_pawns OK", function( assert ) {
  let e = new window.OpenXum.Ordo.Engine();
  e._color = 2;
  let piece = new window.OpenXum.Ordo.Piece(2,new window.OpenXum.Ordo.Coordinates(1,4));
  let mov = new window.OpenXum.Ordo.Move(piece,new window.OpenXum.Ordo.Coordinates(1,5));
  e._check_pawn_taken(mov);
  assert.equal(e._count_white_pawn,19,"On a bien pris un pion");
