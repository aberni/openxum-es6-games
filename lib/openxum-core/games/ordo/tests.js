...
QUnit.test( " _verify_moving starting pos OK", function( assert ) {
  ...
  e._color = 2;
  let list = [];
  list.push(e._board[1][2])
  assert.equal(e._verify_moving(list,1,3) , true,"Move D is possible" );
  assert.equal(e._verify_moving(list,2,2) , true,"Move R is possible" );
  assert.equal(e._verify_moving(list,3,2) , true,"Move R+ is possible" );
  assert.equal(e._verify_moving(list,0,3) , true,"Move DL is possible" );
  assert.equal(e._verify_moving(list,0,2) , false,"Move L isn't possible" );
  assert.equal(e._verify_moving(list,1,1) , false,"Move U isn't possible" );
  assert.equal(e._verify_moving(list,0,1) , false,"Move UL isn't possible" );
  assert.equal(e._verify_moving(list,2,1) , false,"Move UR isn't possible" );
  assert.equal(e._verify_moving(list,2,3) , false,"Move DR isn't possible" );
});
...
QUnit.test( " _verify_moving after mov OK", function( assert ) {
  ...
  e._board[3][3] = new window.OpenXum.Ordo.Piece(col,new window.OpenXum.Ordo.Coordinates(3,3));
  let list = [];
  list.push(e._board[3][3]);
  assert.equal(e._verify_moving(list,2,3) , true,"Move L is possible" );
  assert.equal(e._verify_moving(list,4,3) , true,"Move R is possible" );
  assert.equal(e._verify_moving(list,2,2) , false,"Move UL isn't possible" );
  assert.equal(e._verify_moving(list,3,2) , false,"Move U isn't possible" );
  assert.equal(e._verify_moving(list,4,2) , false,"Move UR isn't possible" );
  assert.equal(e._verify_moving(list,2,4) , false,"Move DL isn't possible" );
  assert.equal(e._verify_moving(list,4,4) , false,"Move DR isn't possible" );
  assert.equal(e._verify_moving(list,3,4) , false,"Move D isn't possible" );
});
...
QUnit.test(" _check_winner with a disconnect black OK ",function(assert) {
  ...
  e.move(mov);
  let movbis = new window.OpenXum.Ordo.Move(e._board[2][7],new window.OpenXum.Ordo.Coordinates(1,7));
  e.move(movbis)
  let movtis = new window.OpenXum.Ordo.Move(e._board[5][2],new window.OpenXum.Ordo.Coordinates(5,3));
  e.move(movtis);
  e.show_board();
  e._check_winner();
  assert.equal(e.winner_is(),1,"Winner is white" );
...
QUnit.test(" _check_winner with a disconnec white OK ",function(assert) {
  ...
  e.move(mov);
  let movbis = new window.OpenXum.Ordo.Move(e._board[0][2],new window.OpenXum.Ordo.Coordinates(0,3));
  e.move(movbis)
  let movtis = new window.OpenXum.Ordo.Move(e._board[5][5],new window.OpenXum.Ordo.Coordinates(5,4));
  e.move(movtis);
  e.show_board();
  e._check_winner();
  assert.equal(e.winner_is(),2,"Winner is black" );
