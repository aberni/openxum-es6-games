...
class Engine extends OpenXum.Engine {
    ...
    clone() {
        ...
        for(let i=0; i<this._stateboard.length; i++) {
            sb.push(this._stateboard[i]);
        }

        let cs = [];
        for(let i=0; i<this._count_state.length; i++) {
            cs.push(this._count_state[i]);
        }

        let p = [];
        for(let i=0; i<this._pawns.length; i++) {
            p.push(this._pawns[i]);
        }

        o._set(this._is_finished, this._winner_color, b, sb, cs, p);

        return o;
    }

    _set(isf, wc, b, sb, cs, p) {
        this._is_finished = isf;
        this._winner_color = wc;
        this._board = b;
        this._stateboard = sb;
        this._count_state = cs;
        this._pawns = p;
    }
    ...
    get_possible_move_list() {
        ...
        let indexPlayer;
        if(this._first_color === this._current_color) {
            indexPlayer = 0;
        } else {
            indexPlayer = 1;
        }
        for(let i=0; i<3; i++) {
            let possible_moves = this._get_possible_move_list(this._pawns[indexPlayer][i]);
    ...
    _initialize_board() {
        ...
        this._board[2][1] = this._pawns[0][0];
        this._board[1][4] = this._pawns[0][1];
        this._board[3][4] = this._pawns[0][2];

        this._board[1][0] = this._pawns[1][0];
        this._board[3][0] = this._pawns[1][1];
        this._board[2][3] = this._pawns[1][2];
    ...
    move(move) {
        ...
        console.assert(piece !== undefined, 'no piece at '+move.from().to_string());
