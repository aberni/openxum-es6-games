...
class Engine extends OpenXum.Engine {
   ...
   * @param first_color couleur du premier joueur
   */
  constructor(type, first_color) {
    ...
    this._stateboard = []; // tableau de tous les états de la partie qui surviennent après l'initialisation
    this._count_state = []; // nombre d'apparition de chaque état dans this._stateboard
    this._pawns = [];
  ...
  /**
   * @brief retourne la couleur du premier joueur
   * @returns {*}
   */
  first_color() {
  ...
  /**
   * @brief retourne la couleur du joueur adverse
   * @returns {number|*}
   */
  opponent_color() {
  ...
  /**
   * @brief Permet d'appliquer une liste de coups.\n
   * Le paramètre moves contient un tableau d'objets Move.
   * @param moves liste des mouvements à appliquer
   */
  apply_moves(moves) {
  }
  ...
  /**
   * @brief Retourne la liste de tous les coups possibles\n
  ...
  /**
   * @brief Retourne si la partie est terminée ou non.
   * @returns {Engine._is_finished}
   */
  is_finished() {
  ...
  /**
   * @brief Modifier l'état du jeu en fonction de l'état passé sous forme d'une chaîne de caractères
  ...
  /**
   * @brief Construit une représentation sous forme d'une chaîne de caractères de l'état du jeu
   */
  to_string() {
  }
  ...
  /**
   * @brief initialise le tableau et les pieces
   * @private
   */
  _initialize_board() {
  ...
  /**
   * @brief Retourne la liste des mouvements possibles pour la piece passée en argument
   * @param piece
   * @returns {Array}
   * @private
   */
  ...
  /**
   * @brief teste l'egalité entre 2 tableaux graces aux chaines de caractères passées en paramètres
   * @param board_string_1
   * @param board_string_2
   * @returns {boolean}
   */
  ...
  /**
   * @brief Retourne la chaine de caractère correspondant au tableau de jeu
   * @returns {string}
   */
  board_to_string() {
    let state = "";
  ...
  /**
   * @brief Teste l'égalité (un état survient 3 fois)
   * @private
   */
  _check_draw() {
  ...
  /**
   * @brief Joue un coup à partir du mouvement passé en paramètre
   * @param move coup à jouer
   */
  move(move) {
    // on 'saisie' la piece
    let piece = this._board[move.from().x()][move.from().y()];
    // Décommenter les asserts pour vérifier que la piece à bouger appartient bien au joueur courant
    ...
    // on met à jour le tableau des pions
    let indexPlayer;
    ...
    // on met à jour le tableau de jeu
    ...
    // on met à jour le joueur gagnant
  ...
  /**
   * @brief Annule le coup passé en paramètre
   * @param move
   */
  undo_move(move) {
