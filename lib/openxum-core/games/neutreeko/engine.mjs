...
class Engine extends OpenXum.Engine {
    ...
    _check_draw()
    {
        ...
        let index = this._stateboard.indexOf(this._board);
        if(index === -1)
        {
            this._stateboard.push(this._board);
            this._count_state.push(1);
        }else {
            this._count_state[index]+=1;
            if(this._count_state[index] === 3)
            {
        ...
        console.log('count_state_length ' + this._count_state.length);
