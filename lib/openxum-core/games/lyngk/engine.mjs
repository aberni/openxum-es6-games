...
class Engine extends OpenXum.Engine {
    ...
    stillOneMovePossible(player) {
        for (let intersection in this._intersections) {
            let neighborsCoord = this._getNeighborsOfCoord(intersection);
            if (neighborsCoord !== false) {
                for (let neighbor in neighborsCoord) {
                    if (this.colorOwnedByPlayer(this._intersections[neighbor.hash()].getColor(), player)) {
                        return true;
                    }
                }
            }
    ...
    winner_is() {
        ...
        for (let intersection in this._intersections) {
            if (intersection.getState() !== State.VACANT) {
                let colorInter = intersection.getColor();
                if (this._colorsPlayer1.indexOf(colorInter) !== -1) {
                    ++nbStackPiecesPlayer1[intersection.getHeightStack() - 1]
                } else {
                    ++nbStackPiecesPlayer2[intersection.getHeightStack() - 1]
                }
            }
        }
    ...
    _isMoveValid(c1, c2) {
        ...
        for (let _neighborCoord in _neighborsCoord) {
            if (_neighborCoord.equals(c2) && !_neighborCoord.equals(c1)) {
                return true;
            }
        }
