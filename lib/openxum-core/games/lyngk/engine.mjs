...
class Engine extends OpenXum.Engine {
    ...
    get_possible_move_list(coord) {
        ...
        if(!coord) {
            //request color
            if(this._availableColors.length > 0 && Math.random() < 0.4) {
                this.colorRequested(Math.floor(Math.random() * this._availableColors.length));
            }
        ...
        if (this._isColorAvailable(color)) {
            switch (this.getCurrentPlayer()) {
                case Player.PLAYER_1 :
                    ...
                    this._availableColors.splice(this._availableColors.indexOf(color), 1);
    ...
    notifyListeners() {
        ...
        this._listeners.forEach(function(listener) {
            listener.availableColorsUpdated(that._availableColors);
        });
    }
    ...
    _reset(){
        ...
        for (let i in this._intersections){
            console.log(this._intersections[i].getState());
        }
    }
