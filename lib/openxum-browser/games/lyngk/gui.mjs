...
class Gui extends OpenXum.Gui {
    ...
    get_move() {
        ...
        move = new Lyngk.Move(this._selected_coordinates, this._target_coordinates);
    ...
    _has_selected_a_piece() {
        return this._selected_coordinates && this._selected_coordinates.isValid();
    }
                ...
                if (number !== -1) {
                    ...
                    if (this._has_selected_a_piece()) {
                        ...
                        if (this._engine._exist_intersection(letter, number)) {
                            let inter = this._engine.getIntersection(null, letter, number);
    ...
    _on_claim_click(selectedColor) {
        if (!this._can_claim_color) {
            return;
        }

        this._can_claim_color = false;
        var result = this._engine.colorRequested(selectedColor);
    ...
    _draw_state() {
        for (let index in this._engine._intersections) {
            ...
            if (intersection.getState() !== Lyngk.State.VACANT) {
                ...
                let isSelected = this._selected_coordinates && this._selected_coordinates.isValid() &&
    ...
    _draw_piece(x, y, intersection, isSelected) {
        ...
        this._context.fillStyle = "#000000";
        this._context.fillText(stackHeight, x, y);
    }
    ...
    _draw_stack() {
        ...
        let radius = this._delta_x * (1.0 / 3 + 1.0 / 10) / 4;
        let startX = 5 + radius;
        let k = 0;
        ...
        for (let i = 0; i < stackColors.length; i++) {
            ...
            this._update_context_style_for_piece(color);
            this._context.beginPath();
            this._context.arc(startX + (++k * radius * 2), startY, radius, 0.0, 2 * Math.PI);
            this._context.closePath();
            this._context.fill();
            this._context.stroke();
        }
    ...
    _draw_possible_moves() {
        ...
        move_list.forEach(function(coords) {
            let letter = coords.getColumn().charCodeAt(0);
            let number = coords.getLine();
            let pt = that._compute_coordinates(letter, number);
            let x = pt[0];
            let y = pt[1];

            that._context.strokeStyle = "#48e723";

            that._context.lineWidth = 4;
            that._context.beginPath();
            that._context.arc(x, y, that._delta_x * (1.0 / 3 + 1.0 / 10), 0.0, 2 * Math.PI);
    ...
    _draw_claimed_colors() {
        this._draw_player_colors(Lyngk.Player.PLAYER_1, 8, 30);
        this._draw_player_colors(Lyngk.Player.PLAYER_2, 8, 60);
    }
    ...
    _draw_player_colors(player, startX, startY) {
        ...
        let radius = this._delta_x * (1.0 / 3 + 1.0 / 10) / 4;
        let k = 0;
        let ownedColors = this._engine.getPlayerColors(player);
        startX += 40;
        startY -= 8;
