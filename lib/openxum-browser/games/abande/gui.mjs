...
class Gui extends OpenXum.Gui {
  ...
  _on_click(event) {
    ...
    if (validate === true) {
      ...
      if (this._engine.current_color() === this._color || this._gui !== null) {
        if (this._engine._intersections[i].getColor() === Abande.Color.AVAILABLE && this._select === false) {
          ...
          right_neighbour = this._engine._opposite_color_neighbour(neighbour, color);
          this._move_type = Abande.Phase.CAPTURE_PIECE;
        }
        else if (this._select === true) {
          if (this._engine._intersections[i].getColor() === Abande.Color.NONE || this._engine._intersections[i].getColor() === Abande.Color.AVAILABLE || (this._engine._intersections[i].getColor() === this._engine.current_color())) {
            this._select = false;
          }
          else {
            for (let n = 0; n < right_neighbour.length; n++) {

              if (right_neighbour[n] === i) {
    ...
    for (let i = 0; i < this._engine._intersections.length; ++i) {
      if (this._engine._hash_exist(i) === true) {
        if (this._select === false) {
          if (this._engine._intersections[i].getColor() === Abande.Color.AVAILABLE) {

            if (this._engine.is_finished() === false) {
              this._draw_possible_move(i);
            }
          }
        }
        if (this._engine._intersections[i].getColor() === Abande.Color.BLACK) {
          this._draw_token(Abande.Color.BLACK, i, this._engine._intersections[i].getStackHeight());
        }
        if (this._engine._intersections[i].getColor() === Abande.Color.WHITE) {
          this._draw_token(Abande.Color.WHITE, i, this._engine._intersections[i].getStackHeight());
        }
      }
    }

    if (this._select === true) {
      for (let i = 0; i < right_neighbour.length; ++i) {
        if (this._engine.is_finished() === false) {
          let c = new Abande.Coordinates();
          c = this._engine._coordinate_from_hash(right_neighbour[i]);

          if (this._engine._can_capture(this._selected_coordinates_from, this._engine._coordinate_from_hash(right_neighbour[i]))) {
            this._draw_possible_capture(right_neighbour[i]);
          }
        }
      }
    }
  ...
  _draw_token(c, i, h) {
    ...
    this._context.arc(x * this._canvas.width / 14, y * this._canvas.height / 8, rayon, 0, 2 * Math.PI);
    this._context.fill();
    ...
    this._context.font = "15pt Calibri,Geneva,Arial";
    this._context.strokeStyle = "#CCCCCC";
    this._context.strokeText(h, (x * this._canvas.width / 14) - rayon / 3, (y * this._canvas.height / 8) + rayon / 3);
    this._context.stroke();
  ...
  _draw_turn() {
    ...
    this._context.beginPath();
    this._context.arc((1 * this._canvas.width / 14) + 10, (0.5 * this._canvas.height / 8) + 10, rayon, 0, 2 * Math.PI);
    ...
    this._context.strokeStyle = "#CCCCCC";
    if (this._engine.black_piece_number() > 9) {
      this._context.strokeText(this._engine.black_piece_number(), (((1 * this._canvas.width / 14) - rayon / 1.25) + 10), ((0.5 * this._canvas.height / 8) + rayon / 2.5) + 10);
    } else {
      this._context.strokeText(this._engine.black_piece_number(), (((1 * this._canvas.width / 14) - rayon / 2.5) + 10), ((0.5 * this._canvas.height / 8) + rayon / 2) + 10);
    }
    this._context.stroke();

    this._context.beginPath();
    this._context.arc((13 * this._canvas.width / 14), (0.5 * this._canvas.height / 8) + 10, rayon, 0, 2 * Math.PI);
    this._context.strokeStyle = "#CCCCCC";
    this._context.stroke();
    ...
    this._context.font = "20pt Calibri,Geneva,Arial";
    this._context.strokeStyle = "#CCCCCC";
    if (this._engine.white_piece_number() > 9) {
      this._context.strokeText(this._engine.white_piece_number(), (((13 * this._canvas.width / 14) - rayon / 0.75 ) + 10), ((0.5 * this._canvas.height / 8) + rayon / 2.5) + 10);
    } else {
      this._context.strokeText(this._engine.white_piece_number(), (((13 * this._canvas.width / 14) - rayon / 1.25 ) + 10), ((0.5 * this._canvas.height / 8) + rayon / 2.5) + 10);
    }
    this._context.stroke();
  }
  ...
  _draw_score() {
    ...
    this._context.font = "20pt Calibri,Geneva,Arial";
    this._context.strokeStyle = "#CCCCCC";
    if (this._engine.count_score()[Abande.Color.BLACK] > 9) {
      this._context.strokeText(this._engine.count_score()[Abande.Color.BLACK], (((1 * this._canvas.width / 14) - rayon / 1.25 ) + 10), ((7.5 * this._canvas.height / 8) + rayon / 2.5) + 10);
    } else {
      this._context.strokeText(this._engine.count_score()[Abande.Color.BLACK], (((1 * this._canvas.width / 14) - rayon / 2.5 ) + 10), ((7.5 * this._canvas.height / 8) + rayon / 2) + 10);
    }
    this._context.stroke();
    ...
    this._context.font = "20pt Calibri,Geneva,Arial";
    this._context.strokeStyle = "#CCCCCC";
    if (this._engine.count_score()[Abande.Color.WHITE] > 9) {
      this._context.strokeText(this._engine.count_score()[Abande.Color.WHITE], (((13 * this._canvas.width / 14) - rayon / 0.75 ) + 10), ((7.5 * this._canvas.height / 8) + rayon / 2.5) + 10);
    } else {
      this._context.strokeText(this._engine.count_score()[Abande.Color.WHITE], (((13 * this._canvas.width / 14) - rayon / 1.25 ) + 10), ((7.5 * this._canvas.height / 8) + rayon / 2.5) + 10);
    }
    this._context.stroke();
  }
