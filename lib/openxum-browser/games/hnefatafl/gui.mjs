...
class Gui extends OpenXum.Gui {
  ...
  set_canvas(c) {
    ...
    this._canvas.addEventListener("click", (e) => {
      ...
      if (pos.x >= 0 && pos.x < 11 && pos.y >= 0 && pos.y < 11) this._on_click(pos.x, pos.y);
    });
  ...
  _on_click(x, y) {
    if (!this._engine.is_finished()) {
      if (this._engine._board[x][y] !== undefined && this._engine._board[x][y].color() === this._engine.current_color()) {
        ...
        //push dnas le tableau
      }
      else {
        //Si le tableau est a 1
        //very moving classique sur une piece
        //sinon on cree un ordo
        //et on appelle very ordo
