...
class Gui extends OpenXum.Gui {
    ...
    _on_click(event) {
            ...
            let id = this._get_id(pos.x, pos.y); // get the id of the cell where the player has clicked
            if (id || id=='0') { // if the id is valid
                console.log(id); // TEST
                if(!this._played) { // if the player hasn't already played
                    this.engine().move(new Move(this._pawn, id)); // send the move to the engine
                    this._played=true;
    ...
    get_move() {
        return new Move(this._moving_id,this._moving_id)
