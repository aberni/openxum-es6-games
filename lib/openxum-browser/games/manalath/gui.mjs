...
class Gui extends OpenXum.Gui {
  ...
  _draw_available_pieces() {
    this._draw_player_pieces(Manalath.Color.WHITE, 8, 30);
    this._draw_player_pieces(Manalath.Color.BLACK, 8, 60);
  }
  ...
  _draw_possible_moves() {
    ...
    let move_list = this._engine.get_possible_move_list(parseInt(color));
    if (!move_list) {
      return;
    }
    ...
    move_list.forEach(function (move) {
      let coords = move.to();
      ...
      that._context.arc(x, y, that._delta_x * (1.0 / 3 + 1.0 / 10) / 2, 0.0, 2 * Math.PI);
      ...
      if (letter !== 'X') {
        ...
        if (number !== -1) {
          ...
          if (this._engine._exist_intersection(letter, number)) {
            ...
            if (inter.is_vacant()) {
              this._selected_color = parseInt($('#colors').val());
