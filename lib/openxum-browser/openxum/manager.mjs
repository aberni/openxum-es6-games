...
// public methods
    ...
    replay(moves, pause) {
        ...
        moves.split(";").forEach(function (move) {
            if (move !== '') {
                _this._move = _this._that.build_move();
                _this._move.parse(move);
                _this._move_list.push(_this._move);
                if (!pause) {
                    _this._engine.move(_this._move);
                }
            }
        });
        if (pause) {
            this._loop = setInterval(this._replay.bind(this), 1000);
        } else {
            this.ready(true);
        }
    }
