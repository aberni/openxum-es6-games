...
class AlphaBetaPlayer extends OpenXum.Player {
  constructor(color, engine, depth) {
    super(color, engine);
  ...
  min(father, depth, maxScore, minScore) {
    ...
    for (let i = 0; i < possible_moves.length; i++) { // For each possible move
      ...
      // Alpha - Beta pruning
      if (minScore <= maxScore || father.is_finished()) {
        //console.log('------------');
        return minScore;
      }
    }
  ...
  max(father, depth, maxScore, minScore) {
    ...
    if (father.is_finished() && father.winner_is() !== this.color()) { // IA loose or DRAW
      this._n_tests++;
      return (-1000 + depth);
    }
    ...
    for (let i = 0; i < possible_moves.length; i++) { // For each possible move
      ...
      // Alpha - Beta pruning
      if (maxScore >= minScore || child.is_finished()) {
        //console.log('++++++++++++');
        return maxScore;
      }
    }
